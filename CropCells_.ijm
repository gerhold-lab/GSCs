...
function getROIs(path) {
	run("Clear Results");
	roiManager("Reset");
	//opens Matlab output which is a txt file with 5 columns - Cell, Frame, X, Y, Z e.g. Cell_10, 11, 84.3, 42.1, 4.2.
	//This file contains the xyz coordinates for the spindle midpoint of all pair centrosomes for all timepoints
	run("Results... ", "open=["+path+"]");
	corresp = replace(path, "_coords.txt", ".tif");
	print(corresp);
	//opens the corresponding tif file for the Matlab output, i.e. the whole germline movie in which the listed cells were tracked
	open(corresp);
	boo = getImageID();
	Stack.getDimensions(width, height, chanels, slices, frames);
	foo = getTitle();
	//name = substring(foo, 0, lengthOf(foo)-4);
	name = replace(foo, ".tif", "");
	print(name);
	//Some tiffs are the processed single channel images and some are the original unprocessed 2 channel images
	//add this step to remove the ani-1::mNG channel
	if (chanels == 2) {
		run("Split Channels");
		selectWindow("C1-"+foo+"");
		close();
	...
	//This opens the Matlab output which is a list of all the cells tracked for a given germline (i.e. Cell_10, Cell_11, etc.)
	//and creates an array in ImageJ called cells wtih this info
	cellindx = replace(path, "_coords.txt", "_cellIDs.txt");
	string = File.openAsString(cellindx);
	//print(string);
	cells = split(string,"\n");
	Array.show(cells);
	//Makes a folder for the created ROIs - 1 folder per germline
	File.makeDirectory(""+ROIs+"\\"+name+"\\"); //PC
	//Makes a folder for the created tiffs - 1 folder per germline
